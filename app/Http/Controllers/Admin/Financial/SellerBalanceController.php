...
class SellerBalanceController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Seller::query()->select('id', 'username', 'updated_at')
            ->where($where);
        ...
        if ($m_ids) {
            $seller_balance_data = SellerBalance::query()->select('m_id', 'amount', 'updated_at')->whereIn('m_id', array_unique($m_ids))->get();
    ...
    public function batchRecharge(Request $request)
    {
        ...
        foreach ($username as $value) {
            $member_id = Seller::query()->where('username', $value)->value('id');
            if (!$member_id) {
                $is_exists[] = $value;
            } else {
        ...
        foreach (array_keys($member) as $val) {
            $res = SellerBalance::updateAmount($val, $amount, SellerBalanceDetail::EVENT_SYSTEM_RECHARGE, '', $note);
            if (!$res['status']) {
    ...
    public function update(Request $request)
    {
        ...
        $note = $request->input('note');
        //验证用户
        if (!Seller::query()->where('id', $id)->exists()) {
            api_error(__('admin.invalid_params'));
        }
        $event = '';
        if ($type == 'recharge') {
            $event = SellerBalanceDetail::EVENT_SYSTEM_RECHARGE;
        } elseif ($type == 'deduct') {
