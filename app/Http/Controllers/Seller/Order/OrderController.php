...
class OrderController extends BaseController
{
    ...
    public function getPrice(Request $request)
    {
        ...
        $data = Order::query()->select('id', 'sell_price_total', 'promotion_price', 'discount_price', 'delivery_price_real', 'subtotal')->where('seller_id', $this->user_data['id'])->find($id);
        if (!$data) {
    ...
    public function updatePrice(Request $request)
    {
        ...
        $order = self::checkOrder($id);
        $res = OrderService::updatePrice($order, $discount_price, $delivery_price_real, $this->user_data, OrderLog::USER_TYPE_ADMIN);
        if ($res === true) {
            return $this->success();
        } elseif ($res === false) {
    ...
    public function getAddress(Request $request)
    {
        ...
        $data = Order::query()->select('id', 'full_name', 'tel', 'prov', 'city', 'area', 'address')->where('seller_id', $this->user_data['id'])->find($id);
        if (!$data) {
            api_error(__('admin.order_error'));
        }
    ...
    public function delivery(Request $request)
    {
        ...
        $res = OrderService::delivery($order, $this->user_data, OrderLog::USER_TYPE_ADMIN, $note, $param);
        if ($res === true) {
            return $this->success();
        } elseif ($res === false) {
            api_error(__('admin.fail'));
    ...
    {
        $order = Order::query()->where('seller_id', $this->user_data['id'])->find($order_id);
        if (!$order) {
            api_error(__('admin.order_error'));
        }
