...
class RecycleController extends BaseController
{
    ...
    public function restore(Request $request)
    {
        ...
        $res = Goods::query()->whereIn('id', $ids)->where('seller_id', $this->seller_id)->restore();
        if ($res) {
            return $this->success();
        } else {
    ...
    {
        $ids = $this->checkBatchId();
        $goods_ids = Goods::onlyTrashed()->whereIn('id', $ids)->where('seller_id', $this->seller_id)->pluck('goods_id')->toArray();
        $res = Goods::completelyDelete($goods_ids);
        if ($res) {
            return $this->success();
        } else {
            api_error(__('admin.del_error'));
