...
class GoodsController extends BaseController
{
    ...
    public function detail(Request $request)
    {
        ...
        $data = Goods::query()->where('seller_id', $this->seller_id)->find($id);
        if (!$data) {
            api_error(__('admin.content_is_empty'));
        }
    ...
    public function status(Request $request)
    {
        ...
        $res = Goods::query()->whereIn('id', $ids)->where('seller_id', $this->seller_id)->update(['status' => $status, 'shelves_status' => Goods::SHELVES_STATUS_OFF]);
        if ($res) {
    ...
    public function shelvesStatus(Request $request)
    {
        ...
        if ($status == Goods::SHELVES_STATUS_ON) {
            $res = Goods::query()->whereIn('id', $ids)->where('seller_id', $this->seller_id)->where('status', Goods::STATUS_ON)->update(['shelves_status' => $status, 'shelves_at' => get_date()]);
            $error_msg = __('admin.goods_shelves_status_fail');
        } else {
            $res = Goods::query()->whereIn('id', $ids)->where('seller_id', $this->seller_id)->update(['shelves_status' => $status]);
    ...
    {
        $ids = $this->checkBatchId();
        $res = Goods::query()->whereIn('id', $ids)->where('seller_id', $this->seller_id)->delete();
        if ($res) {
            Goods::delGoodsCache($ids);//清除商品缓存
            return $this->success();
        } else {
    ...
    public function fieldUpdate(Request $request)
    {
        ...
        $res = Goods::query()->where('id', $id)->where('seller_id', $this->seller_id)->update([$field => $field_value]);
        if ($res) {
            return $this->success();
        } else {
            api_error(__('admin.fail'));
